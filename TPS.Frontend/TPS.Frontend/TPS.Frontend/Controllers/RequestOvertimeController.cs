...
namespace TPS.Frontend.Controllers
{
   ...
   new CancellationTokenSource();
        private readonly IRequestOvertimeService _client;
        private readonly IEmployeeService _employeeService;
        public RequestOvertimeController(IRequestOvertimeService client, IEmployeeService employeeService)
        {
            _client = client;
        ...
        public async Task<ActionResult> ProcessApproval(string id = "")
        {
            ...
            var response = await _client.FindRequestOvertimeAsync(_cancellationTokenSource.Token, accessToken, id);
        ...
        public async Task<IActionResult> GetOvertimeApprovals()
        {
            var accessToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Sid).Value;
            var userId = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier).Value;

            var response = await _client.GetOvertimeApprovalsAsync(_cancellationTokenSource.Token, accessToken, userId);
            switch (response.StatusCode)
            {
                case Infrastructure.StatusCode.Success:
        ...
        public async Task<IActionResult> GetUserRequestsOvertime()
        {
            ...
            var response = await _client.GetPaginatedRequestOvertimeAsync(_cancellationTokenSource.Token, accessToken, paginatedRequest);
            switch (response.StatusCode)
            {
                case Infrastructure.StatusCode.Success:
            ...
            if (id == "")
            {
                ...
                switch (response.StatusCode)
                {
                    case Infrastructure.StatusCode.Success:
                        ...
                        return View(new RequestOvertime
                        {
                            EmployeeId = userId,
        ...
        public async Task<ActionResult> AddOrEdit(string id, RequestOvertime model)
        {
            var accessToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Sid).Value;
            var response = await _client.AddOrEditRequestOvertimeAsync(_cancellationTokenSource.Token, accessToken, model);
        ...
        public async Task<ActionResult> Delete(string id)
        {
            try
            {
                ...
                var response = await _client.DeleteRequestOvertimeAsync(_cancellationTokenSource.Token, accessToken, id);
                switch (response.StatusCode)
                {
                    case Infrastructure.StatusCode.Success:
