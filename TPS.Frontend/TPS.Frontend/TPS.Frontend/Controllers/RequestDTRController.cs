...
namespace TPS.Frontend.Controllers
{
   ...
   new CancellationTokenSource();
        private readonly IRequestDTRService _client;
        private readonly IEmployeeService _employeeService;

        public RequestDTRController(IRequestDTRService client, IEmployeeService employeeService)
        {
        ...
        public async Task<ActionResult> ProcessApproval(string id = "")
        {
            ...
            var response = await _client.FindRequestDTRAsync(_cancellationTokenSource.Token, accessToken, id);
        ...
        public async Task<IActionResult> GetDTRApprovals()
        {
            var accessToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Sid).Value;
            var userId = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier).Value;

            var response = await _client.GetDTRApprovalsAsync(_cancellationTokenSource.Token, accessToken, userId);
            switch (response.StatusCode)
            {
                case Infrastructure.StatusCode.Success:
        ...
        public async Task<IActionResult> GetUserRequestsDTR()
        {
            var accessToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Sid).Value;
            var userId = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier).Value;
            ...
            var response = await _client.GetPaginatedRequestDTRAsync(_cancellationTokenSource.Token, accessToken, paginatedRequest);
            switch (response.StatusCode)
            {
                case Infrastructure.StatusCode.Success:
            ...
            if (id == "")
            {
                ...
                switch (response.StatusCode)
                {
                    case Infrastructure.StatusCode.Success:
                        ...
                        return View(new RequestDtr
                        {
                            EmployeeId = userId,
                            ShiftIn = response.Result.RefShift.ShiftIn,
                            ShiftOut = response.Result.RefShift.ShiftOut,
                            IsNightShift = response.Result.RefShift.IsNightDiff,
                            FirstName = response.Result.FirstName,
                            LastName = response.Result.LastName,
                            ShiftName = response.Result.RefShift.ShiftName,
        ...
        public async Task<ActionResult> AddOrEdit(string id, RequestDtr model)
        {
            var accessToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Sid).Value;
            var response = await _client.AddOrEditRequestDTRAsync(_cancellationTokenSource.Token, accessToken, model);
        ...
        public async Task<ActionResult> Delete(string id)
        {
            try
            {
                ...
                var response = await _client.DeleteRequestDTRAsync(_cancellationTokenSource.Token, accessToken, id);
                switch (response.StatusCode)
                {
                    case Infrastructure.StatusCode.Success:
